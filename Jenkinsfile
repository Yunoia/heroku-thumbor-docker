            ...
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'docker-credentials-api', passwordVariable: 'PASSWORD', usernameVariable: 'USER')]) {
                        def cURL = /curl -n -X PATCH https:\/\/api.heroku.com\/apps\/protagonist-thumbor-stage\/formation -d \'{\"updates\":[{\"type\":\"web\",\"docker_image\": \"${imageId}\"]}\' -H '\"Content-Type: application\/json\"' -H '\"Accept: application\/vnd.heroku+json; version=3.docker-releases\"' -H \'"Authorization: Bearer ${PASSWORD}\"'/
                        println cURL
                        sh cURL
                    }
                }
